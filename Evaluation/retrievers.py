...
def retrieval_bm25(queries,query_ids,documents,doc_ids,excluded_ids,long_context,**kwargs):
    ...
    return all_scores
def retrieval_bm25_with_reasoner(queries,query_ids,documents,doc_ids,excluded_ids,long_context,**kwargs):
    from pyserini import analysis
    from gensim.corpora import Dictionary
    from gensim.models import LuceneBM25Model
    from gensim.similarities import SparseMatrixSimilarity
    ...
    all_scores = {}
    bar = tqdm(queries, desc="BM25 retrieval")
    reasoner_file=kwargs.get('reasoner_file',None)
    with open(reasoner_file,'r') as f:
        query_map=json.load(f)
    queries=[query_map[query] for query in tqdm(queries)]
    for query_id, query in zip(query_ids, queries):

        #query_reasoned=get_llm_response(query)
        #print(query,query_reasoned)
        #print(query_reasoned)
        bar.update(1)
        query = analyzer.analyze(query)
        bm25_query = model[dictionary.doc2bow(query)]
        similarities = bm25_index[bm25_query].tolist()
...
def retrieval_sbert_bge(queries,query_ids,documents,doc_ids,task,instructions,model_id,cache_dir,excluded_ids,long_context,**kwargs):
    ...
    return get_scores(query_ids=query_ids,doc_ids=doc_ids,scores=scores,excluded_ids=excluded_ids)
@torch.no_grad()
def retrieval_sbert_bge_with_reasoner(queries,query_ids,documents,doc_ids,task,instructions,model_id,cache_dir,excluded_ids,long_context,**kwargs):
    #from reasoner_llm_call import get_llm_response
    #from tqdm import tqdm
    with open("question_query_map_cv.json",'r') as f:
        query_map=json.load(f)
    queries=[query_map[query] for query in tqdm(queries)]
    model_id='bge'
    if model_id=='bge':
        model = SentenceTransformer('/home/qinxubo/data/pretrained_models/bge-large-en-v1.5')
        queries = add_instruct_concatenate(texts=queries,task=task,instruction=instructions['query'])
    elif model_id=='sbert':
        model = SentenceTransformer('sentence-transformers/all-mpnet-base-v2')
    else:
...
RETRIEVAL_FUNCS = {
    ...
    'bm25': retrieval_bm25,
    "bm25_with_reasoner":retrieval_bm25_with_reasoner,
    'sbert': retrieval_sbert_bge,
    'bge': retrieval_sbert_bge,
    "bge_with_reasoner":retrieval_sbert_bge_with_reasoner,
